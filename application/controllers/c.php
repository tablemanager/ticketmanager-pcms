...
//        $sql = "SELECT * FROM cmsdb.refund WHERE 1" ;
		...
		if ($mode == "ok"){
			$sql = "SELECT a.*, GROUP_CONCAT(b.orders , \"^^\") AS orders FROM refund_c a LEFT outer JOIN refund_order_c b ON a.cushp = b.cushp where a.state  = 'S'  GROUP BY a.cushp" ;
		} else {
			$sql = "SELECT a.*, GROUP_CONCAT(b.orders , \"^^\") AS orders FROM refund_c a LEFT outer JOIN refund_order_c b ON a.cushp = b.cushp  GROUP BY a.cushp" ;
...
//        );
		...
		if($mode == "chk"){
			$sql = "SELECT * FROM cmsdb.refund_c WHERE chk  = 1 " ;
			$query = $this->rds->query($sql);
			$data['pag_links'] = "이슈발생주문:".$query->num_rows()."건";
		}else if($mode == "calls") {
			$sql = "SELECT * FROM cmsdb.refund_c WHERE calls  = 1 ";
			$query = $this->rds->query($sql);
			$data['pag_links'] = "관심고객:" . $query->num_rows() . "건";
		} else if ($mode == "oks"){
			$sql = "SELECT * FROM cmsdb.refund_c WHERE state  = 'S' ";
			$query = $this->rds->query($sql);
			$data['pag_links'] = "입금확인완료:" . $query->num_rows() . "건";
		}else if($mode != ""){
			$sql = "SELECT * FROM cmsdb.refund_c WHERE state  = '{$mode}' " ;
			$query = $this->rds->query($sql);
			...
			if($cusnm !="" || $cusnm  != null){
				$hsql = "SELECT * FROM cmsdb.refund_order_c WHERE cusnm  like '%{$cusnm}%' " ;
				...
				endforeach;
				$hparr = implode ($pc,',');
				$sql = "SELECT * FROM cmsdb.refund_c WHERE cushp  in ({$hparr})" ;
				$query = $this->rds->query($sql);
				$data['pag_links'] = "";
			}else if($userhp !="" || $userhp  != null) {
				$sql = "SELECT * FROM cmsdb.refund_c WHERE cushp  like '%{$userhp}%' ";
				$data['pag_links'] = "";
				$query = $this->rds->query($sql);
			}else{
				$sql = "SELECT * FROM cmsdb.refund_c";
				$query = $this->rds->query($sql);
        ...
        FORMAT(SUM( IF ( state = 'T' or state = 'S' or state = 'O' or state = 'G', 1, 0 ))*100/count(*),2) as per2
         FROM `refund_c` WHERE  paygu is not null";


		$data['per']= $this->rds->query($perqry)->row();

		$data['title'] = '네이버 고객 명단';
		$data['contentview'] = '/c/s';
		$data['leftview'] = 'left';
		$data['topview'] = 'head';
...
//        );
		...
		$sql = "SELECT * FROM cmsdb.refund_c WHERE id = {$id}";
		$logrow = $this->rds->query($sql)->row();
		$cip = $_SERVER['REMOTE_ADDR'];
		$log = $logrow->log.$cip."/state={$statearr[$gu]}/".date("Y-m-d H:i:s")."\n";

		$sql = "update cmsdb.refund_c set state = '{$gu}', log = '{$log}' where id = {$id} limit 1";
		echo $sql;
		if($this->rds->query($sql)){
			echo  "ok";
		}else{
	...
	function get_sms_type(){
		...
		//			$sql = "SELECT * FROM cmsdb.refund_order_c WHERE cushp  = '{$tel}' " ;
		$sql = "SELECT  COUNT(*) as cnt , SUM(price) AS price , cusnm,  biznm FROM cmsdb.refund_order_c WHERE cushp  = '{$tel}' GROUP BY biznm " ;
		$query = $this->rds->query($sql);
...
안녕하세요. 고객님! 쿠팡 판매사 플레이스엠입니다.

고객님께서 이용하신 $_str 티켓이 오류로 인하여 {$_price}원이 자동환불 되었습니다.
(쿠팡 MY페이지 쿠팡캐시 확인가능)
고객님께 혼선을 드려 죄송한 말씀드리며, 환불 처리된 쿠팡캐시는 회수 예정입니다.

번거로우시겠지만 아래 링크를 클릭하시어 쿠팡캐시 회수에 동의 부탁드립니다.

동의 링크 : http://coupang.sparo.cc/".urlencode(base64_encode($tel))."
...
고객님께서 이용하신 $_str 티켓이 오류로 인하여 {$_price}원이 자동환불 되었습니다.
(쿠팡 MY페이지 쿠팡캐시 확인가능)
고객님께 혼선을 드려 죄송한 말씀드리며, 환불 처리된 금액에 대해 입금요청드립니다.

번거로우시겠지만 안내드리는 계좌번호로 꼭! 입금부탁드립니다.

예금주:(주)플레이스엠 / 은행명:신한은행 / 계좌번호 : 140-008-793313 
...
상담시간 : 09:00~18:00 (점심시간 12:00 ~ 13:00)";
	...
	function s_memo(){
		...
		$sql = "update cmsdb.refund_c set memo = '{$memo}' where id = {$id} limit 1";
		if($this->rds->query($sql)){
			echo  $memo;
		}else{
			echo "err";
