    ...
    <main id="content" class="content" role="main">
        ...
        <div class="row">
            <div class="col-md-12">
                            	...
                            	<div class="form-group">
                                    ...
                                    <div class="col-sm-3">
                                        <select name="chnpick"
                                            ...
                                            <option value="<?=$gpk?>"  <?=($gpk=="PM")?"selected='selected'":"" ?>  ><?=$gpv?></option>
                                            <?php } ?>
                                        </select>
                                    </div>
                                </div>
				...
				<div class="form-group"> 
					<label class="col-sm-3 control-label" for="max-length">메시지 타입 </label>
					<div class="col-sm-8">
						<select id="pcmsitem_msgType"  name="pcmsitem_msgType">
...
?>
						...
						<td style="font-size:12px;">
							<?= $row->msg_type == 1 ? "문자" : ($row->msg_type == 2 ? "알림톡" : "") ?>
						</td>
						<td style="font-size:12px;">
...
//		                            	echo "<a href='./bar/bar_add/$row->id' data-toggle='tooltip' data-placement='right' title='바코드 추가'>"."바코드 추가"."</a></td>";
		                            	echo "<a href='/bar/bar_add/$row->id' data-toggle='tooltip' data-placement='right' title='바코드 추가'>"."바코드 추가"."</a></td>";
		                            }
		                            ?>
		                            <td style="font-size:12px;">
                                    ...
                                    <td><a href = "http://gateway.ticketmanager.ai/internal/cpconfig/<?=$row->gp?>/<?=$row->pcms_id?>" target='_blink' >*</a></td>
				                    <td >
 <?php 
if($row->useyn == 'Y'){
	$uflg =  "사용";
...
$(function(){
	...
	$("#pcmsitem_id").keyup(function() {
		...
		$.ajax({
	        url: "<?php echo site_url('sys/get_msgType'); ?>",
	...
	$('#save_btn').click(function(){
		...
		if ($('#pcmsitem_msgType').val() == '' || $('#pcmsitem_msgType').val() == null) {
			alert('not found');
			$('#pcmsitem_msgType').focus();
			return false;
		}
		if(confirm("등록 하시겠습니까?")){
			...
			var pcmsitem_msgType = $('#pcmsitem_msgType').val();
            $.ajax({
	        url: "<?php echo site_url('bar/kit_add'); ?>",
            data: {chnpick : chnpick , pcmsitem_id : pcmsitem_id , pcmsitem_msgType:pcmsitem_msgType, use_sdate : use_sdate , use_edate : use_edate, textarea_value : textarea_value, pcmsitem_id_arr : pcmsitem_id_arr, curl_headimg : curl_headimg},
            type:"POST",
            success:function(msg)
            {
                //alert(msg);
