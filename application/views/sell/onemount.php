                        ...
                        <form  role="form" id="insertform" name="insertform" class="form-horizontal" role="form" method="post" action="" >
                            <fieldset>
                                    ...
                                    <label class="col-sm-3 control-label" for="simple-big-select">
 									...
 									<div class="col-sm-3">
                                        <select name="factype"
                                                ...
                                                tabindex="-1" >
                                            <option value="0">선택</option>
                                            <?php foreach($factype as $fack => $facv){ ?>
                                            <option value="<?=$fack?>"><?=$facv?></option>
                                            <?php } ?>
                                        </select>
                                    </div>                               
                                </div>
                                <div class="form-group">
                                    <label class="col-sm-3 control-label" for="max-length">권종 코드</label>
                                    <div class="col-sm-7">
                                        <input type="text" name="itemcode" id="itemcode" maxlength="10"
                                               class="form-control"
                                               placeholder="권종코드를 입력해주세요."
                                               data-placement="top">
                                    </div>
                                </div>
                                <div class="form-group">
                                ...
                                <div class="form-group">
                                    ...
                                    <div class="col-sm-7"> 
	                                    <input id="startDate" type="text" class="form-control datetimepicker1" maxlength="10" />
	                                </div>
                                </div>
                                <div class="form-group">
                                    <label class="col-sm-3 control-label" for="max-length">이용종료일</label>
                                    <div class="col-sm-7"> 
	                                    <input id="endDate" type="text" class="form-control datetimepicker1" maxlength="10" />
	                                </div>
                                </div>
...
?>
		                        ...
		                        <tr>
		                            ...
		                            <td><?=$row->itemcode?></td>
		                            <td><?=$row->nm?></td>
		                            <td><?=$factype[$row->factype]?></td>
		                            <td>
		                            	<input id="datemode<?=$row->id?>" type="text" class="datemode form-control datetimepicker1" 
		                                    code="<?=$row->id?>" mode = "sdate" maxlength="10" value="<?=date("m/d/Y",strtotime ($row->sdate))?>" size="10" style="border:0; background-color: transparent;"/>
		                            </td>
		                            <td>
		                            	<input id="datemode<?=$row->id?>" type="text" class="datemode form-control datetimepicker1" 
		                                    code="<?=$row->id?>" mode = "edate" maxlength="10" value="<?=date("m/d/Y",strtotime ($row->edate))?>" size="10" style="border:0; background-color: transparent;"/>
		                            </td>
		                            <td class="hidden-xs">
			                            <div class="btn-group">
...
$(function(){
	...
	$('#selectchn').change(function(){
		...
		$(location).attr('href','/sell/onemount/'+selectchn);
	});	
	...
	$('#save_btn').click(function(){
		...
		if($('#factype').val() == '' || $('#factype').val() == null || $('#factype').val() =='0')
		{
		  alert('사업장을 선택해 주세요.');
		  $('#factype').focus();
		  return false;
		...
		if(confirm("등록 하시겠습니까?")){
            ...
            $.ajax({
	        url: "<?php echo site_url('sell/onemount_add'); ?>",
            ...
            success:function(msg)
            {
                //alert(msg);
      		   ...
      		   $(location).attr('href','/sell/onemount/'+chncode);
